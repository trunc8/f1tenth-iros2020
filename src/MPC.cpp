...
class FG_eval 
{
    public:
        ...
        { 
            ...
            _a_start     = _a_start + _mpc_steps - 1;
        }
        ...
        void LoadParams(const std::map<string, double> &params)
        {
            ...
            _Lf = params.find("l_cg2front") != params.end() ? params.at("l_cg2front") : _Lf;
        ...
        void operator()(ADvector& fg, const ADvector& vars) 
        {
           ...
           // fg[1 + _slip_angle_start] = vars[_slip_angle_start];
...
MPC::MPC() 
{
    // Set default value    
    _mpc_steps = 40;
    _max_steering_angle = 0.4189; // Maximal steering radian (~30 deg)
    _max_accel = 7.51; // Maximal throttle accel
    ...
    _delta_start = _delta_start + _mpc_steps;
    _a_start     = _a_start + _mpc_steps - 1;
    //w_start = w_start + mpc_steps;
    //slip_angle_start = slip_angle_start + mpc_steps;
...
void MPC::LoadParams(const std::map<string, double> &params)
{
    ...
    _max_steering_angle = _params.find("max_steering_angle") != _params.end() ? _params.at("max_steering_angle") : _max_steering_angle;
    _max_accel = _params.find("max_accel") != _params.end() ? _params.at("max_accel") : _max_accel;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) 
{
    ...
    const double slip_angle = state[7];
    // Set the number of model variables (includes both states and inputs).
    // For example: If the state is a 4 element vector, the actuators is a 2
    // element vector and there are 10 timesteps. The number of variables is:
    size_t n_vars = _mpc_steps * 8 + (_mpc_steps - 1) * 2;
    // Set the number of constraints
    size_t n_constraints = _mpc_steps * 8;
    ...
    {
        vars_lowerbound[i] = -_max_steering_angle;
        vars_upperbound[i] = _max_steering_angle;
    }
    ...
    //constraints_lowerbound[_slip_angle_start] = slip_angle;
    ...
    //constraints_upperbound[_slip_angle_start] = slip_angle;
